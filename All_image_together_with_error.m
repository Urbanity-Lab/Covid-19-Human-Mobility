...
% X_K = Xp(:,end);
%   for i= 1:predictionwindow
%       x_k_cu = A(end-(m-1):end,:)*X_K + B(end-(m-1):end,:)*U_1(:,observation_end+1+i);
%       Pred1(:,i) = x_k_cu; % 
%       Update = [Update(:,2:end) x_k_cu]; % Updating 
...
%      grid on 
       ...
       %MeanSquaredError_nc(j) = mean((abs((Actual(i,:) - Pred1(i,:))/Actual(i,:))))*100
     j = j+1;
 end
 
%barcomp = [MeanSquaredError; MeanSquaredError_nc];
% MeanSquaredError_nc = [1.2100 4.2143 1.0438 11.0269 18.6050 11.6681 5.2875 0.9563 0.2503 7.8621 13.6480 10.5298 21.0473 7.7018 15.8899 5.8314 1.9171 3.1658 11.4775 6.8826];
%    MeanSquaredError = [1.1554 2.3475 0.8716 10.8788 14.9775 10.9908 6.9614 4.2074 0.7337 6.4166 13.1763 11.1042 19.5181 7.7872 17.4874 4.2458 0.9588 4.1671 10.4687 11.4291];

% %  
%  for i = 1:20
%     barcomp(i,:) = [MeanSquaredError(i); MeanSquaredError_nc(i)]
% end
% barcomp
% bar(1:20,barcomp')
% legend('Considering 14 day lead-lag','Not Considering lead-lag')
