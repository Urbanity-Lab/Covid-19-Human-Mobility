...
Average_error(el) = mean(MeanSquaredError);
 ...
 axis equal
 title("Number of Delay Coordinates Used: " + (em+1) + ", Error: " + Average_error(el) + "%")
 subtitle("W = 600")
 count(el) = length(find(abs(diag(eigs))>thresh))
 el = el+1;
 end
 ...
 plot(Average_error)
 figure(3)
 bar(16:8:56,count,0.63,'FaceColor',[0 .5 .5],'EdgeColor',[0 .9 .9],'LineWidth',1.5)
 ylim([0 60])
 xlabel('No. of Delay Embedding Used','fontsize',14)
 ylabel('No. of Non-zero Eigenvalues','fontsize',14)
 %% Hankel Function.
