...
%   for i = 1:20
%          Day_actual = Actual(i,2:end)-Actual(i,1:end-1);
%          Day_pred = Pred(i,2:end)-Pred(i,1:end-1);
...
%          figure(1)
%          subplot(5,4,i)
%          plot(abs(movmean(Day_actual(:,1:21)-Day_pred(:,1:21),7)),'-.','linewidth',2)
...
%          %subplot(4,5,j)
% %          subplot(5,4,i)
% %          plot(movmean(Day_pred(:,1:21),7),'r-*','linewidth',2)
% %          xticks([1 7 14 21 28])
...
%          end
%          if i==20
%             lg = legend('Difference between Actual Number of Cases and Prediction','Prediction');
...
%          end
%   end
A = [Actual(:,1) Actual(:,2:end)-Actual(:,1:end-1)];
B = [Pred(:,1) Pred(:,2:end)-Pred(:,1:end-1)]
CE = movmean((A-B),7);
CE_cum = cumsum(abs(CE),2);
for i = 1:20
         filename = ["St.Lucie","Marion",'Lake','Osceola','Collier','Manatee','Sarasota','Seminole','Volusia','Pasco','Brevard','Polk','Lee','Duval','Pinellas','Orange','Hillsborough','Palm','Broward','Miami-Dade'];
          figure(1)
          subplot(5,4,i)
%          bar(abs(Actual(i,:)-Pred(i,:)));
%          hold on
%          plot(abs(Actual(i,:)-Pred(i,:)), 'linewidth',2.5)
         plot(CE_cum(i,:),'r','linewidth',2.5)
         title(sprintf('County: %s', filename(21-i)))
         %xlim([1 28])
         xticks([1 7 14 21 28])
         xticklabels({'June 25','July 02','July 08', 'July 14','July 21'})
         xlabel('Timeline (2020)','Fontweight','bold') 
         ax1 = gca;
         ax = gca;
         ...
         a = annotation(gcf,'textarrow',...
        [ 0 0], [0.7 0.9],...
        'String',{'Cumulation of Error'}, 'HeadStyle', 'none', 'LineStyle', 'none',...
        'FontSize',16, 'color','k','FontName', ...
        'cambria math','FontWeight','bold', 'TextRotation',90);
         end
         %exportgraphics(ax1,'Cumerror.png')
         figure(2)
         subplot(5,4,i)
         plot(CE(i,4:end),'r','linewidth',2.5)
         title(sprintf('County: %s', filename(21-i)))
         %title(sprintf('County: %s', filename(i)))
         hold on
         grid on
         xlim([4 28])
         xticks([4 11 18 25])
         xticklabels({'June 28','July 05','July 12', 'July 19'})
         xlabel('Timeline (2020)','Fontweight','bold') 
         ax2 = gca;
         ...
         a = annotation(gcf,'textarrow',...
        [ 0 0], [0.7 0.9],...
        'String',{'Daily Prediction Error';'(Reported - Prediction)'}, 'HeadStyle', 'none', 'LineStyle', 'none',...
        'FontSize',16, 'color','k','FontName', ...
        'cambria math','FontWeight','bold', 'TextRotation',90);
         end
         %exportgraphics(ax2,'Cumerror.png')
%          if i>16
%              xlabel('Date','Fontsize',14,'Fontweight','bold')
%          end
  end
%exportgraphics(ax2,'Daily.png','Resolution',300)
%exportgraphics(ax1,'Cumerror.png','Resolution',300)
