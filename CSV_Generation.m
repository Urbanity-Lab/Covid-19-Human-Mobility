...
% %% Creating X,X',U and U' matrices
 ...
 for i = 1:20
     ...
     figure(i)
     plot(1:predictionwindow,Actual(i,:),'-*','linewidth',3)
     title(sprintf('County: %s', filename(21-i)))
     hold on
     plot(1:predictionwindow,Pred(i,:),'-*','linewidth',3)
     xticks([1 7 14 21])
     xticklabels({'June 25','July 02','July 08','July 14'})
     xlabel('Data')
     ylabel('Cumulative Infected Case Counts')
     legend('Actual Cumulative Cases','Predicted Cumulative Cases');
     grid on
     filename_actual = sprintf('%s actual.csv',filename(21-i));
     filename_pred = sprintf('%s pred.csv',filename(21-i));
     csvwrite(filename_actual,[MA(i,observation_end-30:observation_end)' ;Actual(i,:)'])
     csvwrite(filename_pred,[MA(i,observation_end-30:observation_end)' ;Pred(i,:)'])
 end
