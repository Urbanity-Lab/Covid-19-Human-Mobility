...
% %% Creating X,X',U and U' matrices
 ...
 [W,eigs] = eig(A);
 %%
 omega=log(diag(eigs));% Compute Cont. Time Eigenvalues
 Freal=imag(omega)./(2*pi);% Compute Frequency
 [T,Im]=sort((1./Freal),'descend');% Sort Frequencies
 figure(1)
 subplot (2,3,el)
 stem(1:50,T(1:50),'b','filled')
 xlabel('Period Serial')
 ylabel('Duration')
 title('1-150 Periods')
 figure(2)
 subplot (2,3,el)
 stem(51:100,T(51:100),'r','filled')
 xlabel('Period Serial')
 ylabel('Duration')
 title('51-100 Periods')
 figure(4)
 subplot (2,3,el)
 stem(101:150,T(101:150),'r','filled')
 xlabel('Period Serial')
 ylabel('Duration')
 title('101-150 Periods')
